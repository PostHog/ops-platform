    ...
    server: {
        handlers: {
            POST: async ({ request }) => {
                ...
                const getSlackMessageBody = (email: string, employeeId: string, managerName: string, jobId: string) => ({
                    "blocks": [
                        {
                            "type": "section",
                            "text": {
                ...
                await Promise.allSettled(jobs.map(async (job) => {
                    try {
                        if (job.queue_name === 'send_keeper_test') {
                            const { employee, manager } = JSON.parse(job.data as string) as KeeperTestJobPayload

                            const userRes = await fetch(`https://slack.com/api/users.lookupByEmail?email=${manager.email}`, {
                                method: 'GET',
                                headers: {
