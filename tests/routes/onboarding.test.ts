...
describe('onboarding route server functions', () => {
  ...
  it('defines getOnboardingHires as a GET server function', () => {
    expect(content).toMatch(/const\s+getOnboardingHires\s*=\s*createAdminFn\(\{/)
  ...
  it('defines getOnboardingRecords as a GET server function', () => {
    expect(content).toMatch(/const\s+getOnboardingRecords\s*=\s*createAdminFn\(\{/)
  ...
  it('defines getDeelEmployeesForPicker as a GET server function', () => {
    expect(content).toMatch(/const\s+getDeelEmployeesForPicker\s*=\s*createAdminFn\(\{/)
  ...
  it('defines createOnboardingRecord as a POST server function with inputValidator', () => {
    expect(content).toMatch(/const\s+createOnboardingRecord\s*=\s*createAdminFn\(\{/)
  ...
  it('defines updateOnboardingStatus as a POST server function with inputValidator', () => {
    expect(content).toMatch(/const\s+updateOnboardingStatus\s*=\s*createAdminFn\(\{/)
  ...
  it('imports OnboardingStatus type from prisma client', () => {
    expect(content).toMatch(/import\s+type\s*\{[^}]*OnboardingStatus[^}]*\}\s*from\s*['"]@prisma\/client['"]/)
  })
})
